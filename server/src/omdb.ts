...
export function isError(response: ByTitleOrIdResponse): response is OmdbError {
    return response.Response === "False";
}

export function isTvshow(response: ByTitleOrIdResponse): response is OmdbTvshow {
    return (response as OmdbTvshow).Type === "series";
}

export function isMovie(response: ByTitleOrIdResponse): response is OmdbTvshow {
    return (response as OmdbTvshow).Type === "movie";
}
...
//function test() {
//    new OmdbApi().bySearch({ s: "deadpool" }).then(t => console.log("done", t.Search.map(t => t.Title)));
//}
