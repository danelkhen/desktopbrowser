...
export async function tsc() {
    for (let project of projects) await exec(`tsc -p ${project}`)
}
...
export async function compileWatch(): Promise<void> {
...
export async function main() {
    try {
        ...
        console.log(action)
        let func = (actions as any)[action] || (exports[action] as Function)
        await func()
        process.exit(0)
    } catch (err) {
        console.log(err)
