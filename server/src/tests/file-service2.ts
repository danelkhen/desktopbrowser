...
// import { Page, SiteConfiguration } from "./config"
// import { Queryable, toQueryable } from "./queryable"
// import { dateToDefaultString, remove } from "./utils"
...
//         let files = this.GetFileAndOrFolders({
//             path: req.Path!,
...
//         if (req.skip != null) files = files.skip(req.skip)
//         if (req.take != null) files = files.take(req.take + 1)
//         return files
//     }
...
//         var parentFiles = orderBy(await yyy.where(t => t.IsFolder).toArray(), t => t.Name)
...
//         var path = req.Path
//         if (String.isNullOrEmpty(path)) return /*new File*/ { IsFolder: true, Path: "", Name: "Home" }
...
//         var path = req.Path
//         if (await IoFile.Exists(path)) await IoFile.Delete(path)
...
//         if (!req.ShowHiddenFiles) files = files.where(t => !t.IsHidden)
//         if (req.HideFolders) files = files.where(t => !t.IsFolder)
//         if (req.HideFiles) files = files.where(t => t.IsFolder)
...
//         if (String.isNullOrEmpty(path) && !this.isWindows()) path = "/"
...
//         if (config != null && config.HomePage != null) return config.HomePage.Files
...
//         if (file2 != null) config.HomePage.Files[remove](file2)
