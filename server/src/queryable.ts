export async function* map<T, R>(list: AsyncIterableIterator<T>, cb: (item: T, index: number) => R): AsyncIterableIterator<R> {
    let index = 0;
...
export async function* filter<T>(list: AsyncIterableIterator<T>, cb: (item: T, index: number) => boolean): AsyncIterableIterator<T> {
    let index = 0;
    for await (let item of list) {
        if (cb(item, index))
            yield item;
