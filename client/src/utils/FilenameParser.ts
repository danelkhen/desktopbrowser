...
export class FilenameParser {
    parse(name: string): FilenameParsedInfo | null {
        ...
        let yearToken = tokens2.find(t => t.type == "year")
        let seToken = tokens2.find(t => t.type == "seasonEpisode")
        let index = tokens2.findIndex(t => t.type == "tag" && t.braces == null)
        let nameTokens = tokens2.slice(index)[takeWhile](t => t.type == "tag" && t.braces == null)
        if (nameTokens.length > 0) x.name = nameTokens.map(t => t.value).join(" ")
        if (yearToken != null) {
        ...
        x.tags = tokens2.filter(t => t.type == "tag" && !nameTokens.includes(t)).map(t => t.value)
        return x
    }
    ...
    tryUInt(token: string): number | null {
        if (!/^[0-9]+$/.test(token)) return null
        return parseInt(token)
    }
    tryYear(token: string): number | null {
        if (!/[0-9][0-9][0-9][0-9]/.test(token)) return null
        let x = parseInt(token)
        if (x > 1800 && x < 3000) return x
        return null
    }
    trySeasonEpisode(token: string): { season: number; episode: number } | null {
        if (token.length > 6) return null
        let tests: RegExp[] = [/[sS]([0-9][0-9])[eE]([0-9][0-9])+/, /[sS]([0-9][0-9])/, /([0-9])x([0-9])+/]
