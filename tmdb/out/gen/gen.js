...
function postProcess(methodss) {
    ...
    let methods = serviceClass.members.filter(t => t instanceof Method);
    methods.forEach(method => {
        let ce = new Type();
        let name = method.name[0].toUpperCase() + method.name.substr(1) + "Request";
        ce.name = name;
        ce.members.push(...method.parameters.map(t => new Property(t)));
        ce.members.forEach(t => t.isOptional = true);
        _module.members.push(ce);
        method.parameters = [new Parameter({ name: "req", type: getTypeRef(ce) })];
    });
    let types = _module.members.filter(t => t instanceof Type);
    let skip = new Set();
    let duplicates = []; //{ ce: Type, duplicates: Type[] }[] = [];
    types.forEach(ce1 => {
        if (skip.has(ce1))
            return;
        let entry = new Set(); //{ ce: ce1, duplicates: new Array<Type>(), all:Type[] };
    ...
    duplicates.forEach(set => {
        ...
        _module.members = _module.members.filter(t => !set.has(t));
        _module.members.push(ce);
        //console.log(ce);
    });
    _module.members.filter(t => t instanceof Type).forEach(ce => ce.members.sort((a, b) => a.name.localeCompare(b.name)));
    _module.members.push(new Variable({ name: "Metadata", value: JSON.stringify(_md, exceptParentReplacer, TAB) }));
}
function isTypeRefEquals(x, y) {
...
function normalizeDoc(doc) {
    ...
    doc.pathsList.forEach(path => {
        path.value.methods = normalizeHash(path.value, ["get", "post", "put", "delete"]);
...
function getUniqueName(baseName, set) {
    if (set == null)
        set = _usedNames;
    let name = baseName;
    let index = 1;
...
function processMethod(me, path2) {
    ...
    let retType = me.responses["200"] == null ? { type: "any" } : me.responses["200"].schemaObj;
    //let retCe = 
...
function getTypeRef(ce, typeArgs) {
    let tr = new TypeRef();
    tr.name = ce.name;
    tr.type = ce;
    tr.args = typeArgs || [];
...
function processProperty(pe, className) {
...
function toList(obj) {
    return Object.keys(obj).map(t => ({ key: t, value: obj[t] }));
}
