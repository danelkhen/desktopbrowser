...
function xhrGetJson(url: string): Promise<any> {
    return new Promise<void>((resolve, reject) => {
        ...
        xhr.open("GET", url)
        ;(xhr.onload = () => resolve(JSON.parse(xhr.response))), (xhr.onerror = () => reject(xhr.response)), xhr.send()
    })
}

async function main() {
    //xhrGetJson("../../tmdb.v3.swagger.json").then(e => _doc = e).then(main2);
    const e = await xhrGetJson("../../tmdb.v4.swagger.json")
    _doc = e
    main2()
}
...
function postProcess(methodss: Method[][]) {
    ...
    ;(_module.members.filter(t => t instanceof Type) as Type[]).forEach(ce =>
        ce.members.sort((a, b) => a.name.localeCompare(b.name))
    )
...
function isTypeRefEquals(x: TypeRef, y: TypeRef): boolean {
    if (x.type != null && y.type != null) {
        if (!isTypeEquals(x.type, y.type)) return false
    } else if (x.name != y.name) return false
    if (x.args.length != y.args.length) return false
    if (x.args.find((t, i) => !isTypeRefEquals(t, y.args[i])) != null) return false
    return true
}
...
function isTypeEquals(x: Type, y: Type): boolean {
    ...
    for (let me1 of x.members) {
        ...
        if (me2 == null) return false
        let tr1 = me1.type
        let tr2 = me2.type
        if (!isTypeRefEquals(tr1, tr2)) return false
    }
    return true
}
...
function normalizeHash<T>(obj: Hash<T>, keys?: string[]): KeyValue<T>[] {
    ...
    return keys
        .map(key => {
            ...
            if (typeof value != "object") return null //value = { value } as any as T;
            return { key, value }
        })
        .filter(t => t != null)
}
...
function normalizeDoc(doc: SwaggerDoc) {
    ...
    doc.pathsList.forEach(path => {
        ...
        path.value.methods.forEach(method => {
            ...
            method.value.responsesList.forEach(
                response => (response.value.schemaObj = normalizeDef(response.value.schema))
            )
        })
    })
    addParents(doc)
...
function getBraces(s: string) {
    ...
    res.missing = Array.from(res.missing)
        .reverse()
        .join("")
    //res.missing
    return res
...
function normalizeDef(def2: SwaggerDef | string): SwaggerDef {
    ...
    if (typeof def2 == "string") {
        ...
        } else def = { type: def2, propertiesList: [], properties: null, items: null, _parent: null }
    } else {
...
function tokensToString(tokens: any, sb: string[]) {
    ...
    if (tokens instanceof Array) tokens.forEach(t => tokensToString(t, sb))
    else {
        sb.push(tokens)
    }
}
...
function processDef(ce: SwaggerDef, name?: string): TypeRef {
    if (ce.type == "object") {
        ...
        if (ce3 != null) console.warn("two classes with the same name found", ce2.name, ce2, ce3)
